...
export default function Home() {
  ...
  const handleGeneration = async (topic: string) => {
    ...
    try {
        // For guest users, the roadmap is not in the DB. Save it to session storage
        // so the roadmap page can load it without a DB call.
        if (!("userId" in result)) {
             sessionStorage.setItem(`temp_roadmap_${result.id}`, JSON.stringify(result));
        }
       
        // ALWAYS save metadata to local storage for the history page.
        const historyItem = {
            id: result.id,
            title: result.title,
            query: topic.trim(),
        ...
        toast({
            variant: 'destructive',
            title: 'Browser Error',
            description: 'Could not save the roadmap in your browser. Local storage may be full or disabled.',
        });
        setIsLoading(false);
    }
  };
  ...
  return (
    <main className="flex flex-col items-center px-4 pt-12 sm:px-6 lg:px-8 relative overflow-x-hidden">
      ...
      <section className="w-full max-w-3xl text-center min-h-[calc(100vh-8rem)] flex flex-col justify-center items-center">
        ...
        <p className="text-xl font-medium text-primary mt-2">by QuantumStack X Valndor</p>
